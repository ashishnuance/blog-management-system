    ...
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                ...
                <div class="card-footer p-4 imageVideoPreview" style="display: none">
...
<script type="text/javascript">
    ...
    resumable.on('fileSuccess', function (file, response) { // trigger when file upload complete
        ...
        mediaFileId.push(response.media_id);
        console.log('mediaFileId',mediaFileId);
        let imageVideoPreview = $('.imageVideoPreview');

        if (response.mime_type.includes("image")) {

            imageVideoPreview.append(`<img id="imagePreview" src="${response.path + '/' + response.name}" style="width: 20%; height: auto;" alt="img"/><span class="removeMedia" data-mediaid="${response.media_id}">Remove</span>`);//.attr('src', response.path + '/' + response.name).show();
        }

        if (response.mime_type.includes("video")) {
            imageVideoPreview.append(`<video id="videoPreview" src="${response.path + '/' + response.name}" controls style="width: 100%; height: auto;"></video><span class="removeMedia" data-mediaid="${response.media_id}">Remove</span>`);
    ...
    $('body').on('click','.removeMedia',function(){
        let mediaId = $(this).data('mediaid');
        $.ajax({
            url:"{{route('file-delete')}}",
            type:'post',
            data:{_token: '{{ csrf_token() }}','mediaId':mediaId},
